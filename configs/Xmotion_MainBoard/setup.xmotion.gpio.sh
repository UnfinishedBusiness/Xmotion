...
for FILE in /sys/devices/ocp.*/xmotion_pwm_J[234]_pinmux.*/state ; do
	if [ -f $FILE ] ; then
...
FILE=/sys/devices/ocp.*/xmotion_leds.*/leds/xmotion\:status_led/trigger
if [ -f $FILE ] ; then
...
export_gpio <<- EOF
	...
	61	low	# p8.26		gpio1.29
	89	low	# p9_41A	gpio0.7
EOF
